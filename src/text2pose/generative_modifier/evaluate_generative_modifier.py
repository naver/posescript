...
from text2pose.generative_B.evaluate_generative_B import load_model as load_pose_model
from text2pose.retrieval_modifier.evaluate_retrieval_modifier import load_model as load_textret_model
from text2pose.fid import FID

os.environ['TOKENIZERS_PARALLELISM'] = 'false'

CONTROL_MEASURES = ["GT", "random", "auto_posefix-A_cap0"]
...
def load_model(model_path, device):
	...
	# load model
	model = FeedbackGenerator(text_decoder_name=text_decoder_name,
								comparison_module_mode=comparison_module_mode,
								...
								comparison_latentD=comparison_latentD,
...
def eval_model(model_path, dataset_version, pose_model_version=None, fid_version=None, textret_model_version=None, split='val'):
	...
	# define result file
	get_res_file = evaluate.get_result_filepath_func(model_path, split, dataset_version, precision, controled_task="modifier_generation")
	...
	if not os.path.isfile(filename_res) or OVERWRITE_RESULT:
		if control_measures and "auto" in control_measures:
			...
			d = PoseFix(version=dataset_version, split=split, tokenizer_name=tokenizer_name, caption_index=0, num_body_joints=model.pose_encoder.num_body_joints, cache=True) # caption_index=0: will yield the first reference text
...
def get_evaluation_model_paths(pose_model_version, fid_version, textret_model_version, seed=1):
	...
	if pose_model_version is not None:
		...
		print("Using the following A-and-modifier-to-B generative model for evaluation:", pose_model_path)
	...
	if textret_model_version is not None:
		...
		assert os.path.isfile(textret_model_path), "Text-to-pose-pair retrieval model file not found: " + textret_model_path
...
def prepare_dataset_for_auto_control_metric(split, tokenizer_name, original_dataset_version, auto_specification):
	...
	d = PoseFix(version=auto_dataset_version, split=split, tokenizer_name=tokenizer_name, caption_index=1, cache=True)  # caption_index must be 1 to yield a reference text later on  
	# get the subset of items corresponding to the reference version (ie. to get the reference text to compare against)
	d_query = PoseFix(version=original_dataset_version, split=split, tokenizer_name=tokenizer_name, cache=True)
	d_query_data = {dc[-3]: [dc[2], dc[3]] for dc in d_query._data_cache} # {data_ID: (caption_tokens_list, caption_length_list)}
	# update the cache
	# [the poses; the text tokens & lengths (order: the automatic text of interest + the reference texts); the pair ID; the poses IDs]
	d._data_cache = [[
		dc[0], dc[1], # poses A & B
		[dc[1][auto_cap_index]]+d_query_data[dc[-3]][0], # text tokens: the automatic text of interest + the reference texts
		[dc[2][auto_cap_index]]+d_query_data[dc[-3]][1], # text lengths idem
		dc[4], dc[5], dc[6]] # IDs (pair ID, pose A ID, pose B ID)
		for dc in d._data_cache if dc[-3] in d_query_data] # limit to the subset of interest
	...
	# automatic text
	def update_dict(d, reference_modifier):
		d['modifier'] = [d['modifier'][auto_cap_index]] + reference_modifier # the raw automatic text of interest + the raw reference texts
		return d
	d_query._load_data()
	d._load_data()
	d.dataIDs = list(d_query_data.keys()) # limit to the subset of interest
	d.triplets = {data_id: update_dict(d.triplets[data_id], d_query.triplets[data_id]['modifier']) for data_id in d.dataIDs}
	...
	for i_batch, item in tqdm(enumerate(data_loader)):
		...
		with torch.inference_mode():				
			...
			if control_measures:
				...
				# generate text
				decoded_texts, likelihood_scores = model.generate_text(poses_A, poses_B)
			...
			if pose_model is not None:
				...
				model_input = dict(poses_A=poses_A,
					   				poses_B=poses_B,
				...
				output_original = pose_model.forward(poses_A, caption_tokens_original, caption_lengths_original, poses_B)
				
				# ... with the generated text
				# & compute the pose metrics
				pose_metrics, output = evaluate.add_elbo_and_reconstruction(model_input, pose_metrics, pose_model, body_model, output_distr_key="q_f", reference_pose_key="poses_B")
				...
				# kld metrics
				kld = torch.sum(torch.distributions.kl.kl_divergence(output['q_z'], output['q_f']), dim=[1]) # (this_batch_size)
				kld_original = torch.sum(torch.distributions.kl.kl_divergence(output_original['q_z'], output_original['q_f']), dim=[1]) # (this_batch_size)
				results['kld_a'] += torch.distributions.kl.kl_divergence(output_original['q_f'], output['q_f']).sum().detach().item() # sum over features & batch
			...
			if textret_model:
				...
				# compute embeddings
				pose_embs, text_embs = textret_model(poses_A, caption_tokens_, caption_lengths_, poses_B)
...
if __name__ == '__main__':
	...
	evaluate.eval_parser.add_argument('--pose_generative_model', type=str, help="Shortname of the pose generative model to use for circle evaluation.")
	evaluate.eval_parser.add_argument('--textret_model', type=str, help="Shortname of the text-to-pose-pair retrieval model to use for computing top R-precision metrics.")
